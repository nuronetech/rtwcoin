...
#if defined(NDEBUG)
#error "AriA cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uaria) are considered zero fee (for relaying and mining)
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zARA spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zARA spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zARA spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
      ...
      if (nHeight == 0) {
          nSubsidy = 2000000 * COIN; //cut for AriA network[high price]
      } else if (nHeight == 3) {
          nSubsidy = 150000000 * COIN; //this for Premine/Ico
      } else if (nHeight <= 100 && nHeight > 3) {
          nSubsidy = 10 * COIN; 
      } else if (nHeight <= 500 && nHeight > 100) {
          nSubsidy = 3 * COIN; 
      } else if (nHeight <= 5000 && nHeight > 500) {
          nSubsidy = 0.3 * COIN;
      } else if (nHeight <= 50000 && nHeight > 5000) {
          nSubsidy = 0.13 * COIN;
      } else if (nHeight <= 100000 && nHeight > 5000) {
          nSubsidy = 0.01 * COIN;
      } else if (nHeight <= 500000 && nHeight > 100000) {
          nSubsidy = 0.001 * COIN;
      } else if (nHeight <= 15000000 && nHeight > 500000) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from AriA
...
void RecalculateZAriASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zARA supply
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zARA with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zARASpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ARA to zARA
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zARA spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zARA serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zARA mints and %d zARA spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // AriA: We use certain sporks during IBD, so check to see if they are
