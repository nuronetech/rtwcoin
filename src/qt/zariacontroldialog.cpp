...
#include "ui_zariacontroldialog.h"
...
std::list<std::string> ZAriAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAriAControlDialog::listMints;

ZAriAControlDialog::ZAriAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAriAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAriAControlDialog::~ZAriAControlDialog()
{
    delete ui;
}

void ZAriAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAriAControlDialog::updateList()
{
...
void ZAriAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAriAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAriA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAriAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAriAControlDialog::GetSelectedMints()
{
...
void ZAriAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
